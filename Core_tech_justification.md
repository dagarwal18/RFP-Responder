...
- **Fast & Deterministic Extraction (`qwen/qwen3-32b` via Groq):** For structural tasks like Section Structuring (A2), Go/No-Go (A3), and Requirement Extraction (B1), we use Qwen3-32B (configured at `temperature=0, seed=42`). It excels at adhering to strict JSON output formats under a ~32K context window budget. Groq’s LPU inference engine ensures responses are returned in milliseconds. To sustain this, we implemented an advanced `KeyRotator` singleton to manage strict 6K/30K Tokens-Per-Minute (TPM) limits across multiple API keys using a sliding-window token bucket algorithm.
- **Deep Narrative & Validation (`meta-llama/llama-4-scout-17b-16e-instruct`):** Writing prose (C2, C3) and cross-checking large documents (D1, E1, E2) requires immense context windows. Llama 4 Scout provides a ~131K context length, allowing the system to consume an entire 50-page RFP and its corresponding response simultaneously to flag contradictions. We built a dynamic batching mechanism for C2 to respect the ~25K input budget on Llama, preventing silent context truncation.
