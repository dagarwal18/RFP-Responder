...
class ReviewService:
    ...
    def _build_response_sections(state: RFPGraphState) -> list[ReviewSection]:
        ...
        assembled_sections = ReviewService._extract_assembled_section_map(
            state.assembled_proposal.full_narrative or ""
        )
        ...
        for response in sorted_responses:
            ...
            assembled_content = assembled_sections.get(
                ReviewService._normalize_heading_title(response.title)
            )
            if assembled_content:
                assembled_content = ReviewService._sanitize_response_text(assembled_content)
                if assembled_content and not (
                    ReviewService._is_stub_text(assembled_content) and content.strip()
                ):
    ...
    def _normalize_heading_title(title: str) -> str:
        ...
        normalized = _HEADING_NUMBER_RE.sub("", normalized)
    ...
    def _is_stub_text(text: str) -> bool:
        ...
        return "pipeline_stub" in lowered or "content generated by dedicated agent" in lowered

    @staticmethod
    def _extract_assembled_section_map(markdown: str) -> dict[str, str]:
        ...
        def flush_current() -> None:
            ...
            existing = section_map.get(current_title, "")
            if (
                current_title not in section_map
                or "```mermaid" in content
                or len(content) > len(existing)
            ):
                section_map[current_title] = content
            current_lines = []
        ...
        for line in text.splitlines():
            ...
            if match:
                flush_current()
                current_title = ReviewService._normalize_heading_title(match.group(2))
                continue
