...
class ReviewService:
    ...
    def _build_response_sections(state: RFPGraphState) -> list[ReviewSection]:
        ...
        for response in sorted_responses:
            ...
            if "content generated by dedicated agent" in content.lower() or "PIPELINE_STUB" in content:
                continue
        ...
        commercial_text = _get(getattr(state, "commercial_result", None), "commercial_narrative", "") or ""
        ...
        legal_text = _get(getattr(state, "legal_result", None), "legal_narrative", "") or \
                     _get(getattr(state, "legal_result", None), "risk_register_summary", "") or ""
        if legal_text.strip():
            legal_paragraphs = [
                ReviewParagraph(
